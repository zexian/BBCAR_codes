...
w_matrix = read.table('/projects/p30007/Zexian/Alignment/BBCAR_NEW/administrative/Step21data/N_mutations.txt',sep='\t',header=TRUE)
...
clinical_sort<-clinical[match(gsub('\'','',w_matrix$Name),clinical$Study_ID),]
...
temp$subtype<-ifelse(temp$Basal==1,'TNBCs',ifelse(temp$HER2==1,'Her2',ifelse(temp$CaseControl=='Control','Control','Other Cancer')))


temp$Name <- factor(temp$Name, levels = temp$Name[order(-temp$Number)])

tiff('/projects/p30007/Zexian/Alignment/BBCAR_NEW/administrative/Step21data/BarPlot_Mutation_Number.tif',width = 1600, height = 800, units = 'px', res = 180)
p<-ggplot(temp, aes(x=Name, y=Number, fill=subtype)) +
...
temp<-cbind.data.frame(w_matrix,clinical_sort)
temp_select<-temp[temp$ER_Sta%in%c('Positive','Negative'),]
temp_select$`ER Status`<-droplevels(temp_select$ER_Sta)
tiff("/projects/p30007/Zexian/Alignment/BBCAR_NEW/administrative/Step21data/Mutation_interval.tif", width = 1600, height = 800,units='px',res=180)
P1<-ggplot(temp_select,aes(interval_days,Number,color=`ER Status`))+labs(colour="ER Staus")+
    geom_point()+
...
tiff("/projects/p30007/Zexian/Alignment/BBCAR_NEW/administrative/Step21data/Mutation_interval_30.tif", width = 1600, height = 800,units='px',res=180)
P1<-ggplot(temp_select,aes(interval_days,Number30,color=`ER Status`))+labs(colour="ER Staus")+
    geom_point()+
    geom_smooth(method = "lm", se = FALSE) + 
    #scale_x_reverse()+
    theme(axis.title.x=element_blank(),axis.title.y=element_blank())+ 
...
#now calculate the number of mutations for different tables 
library(plyr)
...
Case<-combined_df[combined_df$CaseControl=='Case',]
Control<-combined_df[combined_df$CaseControl=='Control',]

df_Case <- count(Case, c('sample'))
df_Control <- count(Control, c('sample'))

mean(df_Case$freq)
sd(df_Case$freq)

mean(df_Control$freq)
sd(df_Control$freq)


Alf_Case<-aggregate(Case$ALE, list(Case$sample), mean)
Alf_Control<-aggregate(Control$ALE, list(Control$sample), mean)
...
library(gmodels)
CrossTable(combined_df$variant_type,combined_df$CaseControl,prop.t=FALSE, prop.r=FALSE, prop.c=TRUE,chisq=TRUE,prop.chisq=FALSE )
CrossTable(combined_df$significance,combined_df$CaseControl,prop.t=FALSE, prop.r=FALSE, prop.c=TRUE,chisq=TRUE,prop.chisq=FALSE )
...
tem_cancer<-temp[temp$CaseControl=='Case',]
tem_cancer<-temp[temp$Basal==1 | temp$CaseControl=='Control' ,]
fit<-glm(as.factor(Basal)~V1,data=temp,family = 'binomial')
summary(fit)
fit<-glm(as.factor(Basal)~V1+Meno+Benign_Age+Class,data=temp,family = 'binomial')
summary(fit)
